export default function ({ Plugin, types: t }) {
  const TYPES = /(controller|config|service|filter|animation|provider|directive|factory)/;
  ...
  function annotateModuleConfigFunction(path) {
    ...
    if (moduleLastArg && moduleLastArg.isFunctionExpression()) {
  ...
  return new Plugin('angular-annotate', {
    visitor: {
      MemberExpression() {
        if (this.matchesPattern('angular.module')) {
