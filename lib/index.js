export default function ({ Plugin, types: t }) {
  ...
  return new Plugin('angular-annotate', {
    visitor: {
      CallExpression() {
        if (this.get('callee.object').isCallExpression() && this.get('callee.object.callee').matchesPattern('angular.module')) {
          if (this.get('callee').get('property').isIdentifier({ name: 'controller' })) {
            ...
            let func = this.get('arguments')[1];
            annotateFunction(func);
          } else if (this.get('callee').get('property').isIdentifier({ name: 'animation' })) {
            let func = this.get('arguments')[1];
            annotateFunction(func);
